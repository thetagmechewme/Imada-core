...
#if defined(NDEBUG)
#error "IMADA cannot be compiled without assertions."
#endif
...
std::unique_ptr<ImadaState> globalState = nullptr;
...
std::string SCVersion ("/Imadacore:1.0.0/");
...
unsigned int dgpMaxBlockSize = 2000000; // imada
...
const string strMessageMagic = "Imada Signed Message:\n";
...
#                   endif
        ...
        if(chainActive.Height() >= chainparams.FirstSCBlock() && tx.HasCreateOrCall()) {
            ...
            ImadaDGP imadaDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = imadaDGP.getMinGasPrice(chainActive.Height() + 1);
            uint64_t blockGasLimit = imadaDGP.getBlockGasLimit(chainActive.Height() + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            ImadaTxConverter converter(tx, NULL);
            ExtractImadaTX resultConverter;
            if(!converter.extractionImadaTransactions(resultConverter)){
            ...
            std::vector<ImadaTransaction> imadaTransactions = resultConverter.first;
            std::vector<EthTransactionParams> imadaETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(ImadaTransaction imadaTransaction : imadaTransactions){
                sumGas += imadaTransaction.gas() * imadaTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, imadaTransaction.gasPrice());
                } else {
                    txMinGasPrice = imadaTransaction.gasPrice();
                }
                VersionVM v = imadaTransaction.getVersion();
                if(v.format!=0)
                ...
                if(imadaTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(imadaTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(imadaTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += imadaTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)imadaTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(imadaETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > imadaTransactions.size())
...
//#if 0
      if (pindex->nHeight > Params().FirstSCBlock()) {
        globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // imada
        globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // imada
...
#if 0
    ImadaDGP imadaDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setImadaSchedule(imadaDGP.getGasSchedule(pindex->nHeight + 1));
#endif
    uint64_t minGasPrice = 0;//imadaDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = DEFAULT_BLOCK_GAS_LIMIT_DGP;//imadaDGP.getBlockGasLimit(pindex->nHeight + 1);

#if 0
    uint32_t sizeBlockDGP = 0;//imadaDGP.getBlockSize(pindex->nHeight + 1);
...
///////////////////////////////////////////////////////////////////////////////////////// imada
        if (pindex->nHeight >= Params().FirstSCBlock()) {
            ...
            if(tx.HasCreateOrCall() && !hasOpSpend) {
                ...
                ImadaTxConverter convert(tx, &view, &block.vtx);

                ExtractImadaTX resultConvertImadaTX;
                if(!convert.extractionImadaTransactions(resultConvertImadaTX)) {
                ...
                if(!CheckMinGasPrice(resultConvertImadaTX.second, minGasPrice))
                ...
                dev::u256 gasAllTxs = dev::u256(0);
                ByteCodeExec exec(block, resultConvertImadaTX.first, blockGasLimit);
                ...
                for(ImadaTransaction& ltx : resultConvertImadaTX.first) {
                ...
                {
                    for(size_t k = 0; k < resultConvertImadaTX.first.size(); k ++){
                        ...
                        tri.push_back(TransactionReceiptInfo{block.GetHash(pindex->nHeight >= Params().SwitchPhi2Block()), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertImadaTX.first[k].from(), resultConvertImadaTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const CBlock* pblock)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        if (pindexNew->nHeight >= chainparams.FirstSCBlock()) {
            if (globalState == nullptr)
                return error("%s: globalState is not yet initialized!\n", __func__);
            oldHashStateRoot = dev::h256(globalState->rootHash()); // imada
            oldHashUTXORoot = dev::h256(globalState->rootHashUTXO()); // imada
        }
        ...
        if (!rv) {
            ...
            if (pindexNew->nHeight >= chainparams.FirstSCBlock()) {
                globalState->setRoot(oldHashStateRoot); // imada
                globalState->setRootUTXO(oldHashUTXORoot); // imada
...
bool CheckForMasternodePayment(const CTransaction& tx, const CBlockHeader& header) {
    ...
    // Divide to keep a check precision of 0.01 IMADA
 ...
 * As an example, Imadacore uses /Imadacore:X.X.X/ version string. So, this function will
 * return 5 for /Imadacore:5.0.0/ or 0 if it couldn't find ":" or "." delimeters, or if no digits are between
 * those delimeters, or if string verison is empty
 * @param cleanVersion Imadacore wallet version as a string
 * @return Major Imadacore major version or 0
 */
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    ImadaTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<ImadaTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path imadaDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(imadaDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(imadaDir.string(), std::ios::in | std::ios::out);
...
bool ImadaTxConverter::extractionImadaTransactions(ExtractImadaTX& imadatx){
    std::vector<ImadaTransaction> resultTX;
    ...
    imadatx = std::make_pair(resultTX, resultETP);
    return true;
}

bool ImadaTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool ImadaTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
ImadaTransaction ImadaTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    ImadaTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = ImadaTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = ImadaTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
